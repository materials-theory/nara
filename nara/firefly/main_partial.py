...
from scipy.special import softmax # for PAM
...
class ASEAgent(BaseAgent):
    ...
    def evaluate(self):
        if self._fitness is None:
            self._fitness = self._x.get_potential_energy()/len(self._x) # per atom energy
...
< Firefly algorithms parameters >
 ...
 # ({self.current_gen.best_agent.label})\n\n")
    ...
    def evolve(self, frozen_index = None) -> bool:
        ...
        As = np.vstack([Is]*self.population_size)
        # As = Is*np.exp(-self.gamma*Ds**2)


        if len(frozen_index) > 0:
            As[:, frozen_index] = -np.inf
        np.fill_diagonal(As, -np.inf)

        p_Js = softmax(As, axis=1)
        idx_Js = np.array([np.random.choice(self.population_size, p=p) for p in p_Js], dtype=int)
        ...
        for i, j in zip(idx_Is, idx_Js):
            if i in frozen_index:
                ...
                if i==j:
                    ...
                    dx = (agent_j - agent_i)
                    dx *= self.beta_0 * np.exp(-self.gamma * Ds[i,j]**2)
                _dxes.append(dx)
...
class FA_AL(FA):
    def __init__(self,
                 ...
                 restart: Optional[str] = None,  # filename of pickle file  # TODO - DFT data:xyz 파일 읽는 것도 넣어두자
    ...
    < Firefly algorithms parameters >
        ...
        * verbosity = {self.verbosity} # 0(error) < 1(warning) < 2(info) < 3(debug) \n\n
        ...
        # 3-2) Restart file 없으면 Gen1 부터 시작
        if self.current_gen is None:
            ...
            if not load_gnn:
                ...
                self.re_train(geo_opt=True, max_steps=None) # self.train_atoms, self.valid_atoms, lcb update
                # Full local opt here
