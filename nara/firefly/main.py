...
class ASEAgent(BaseAgent):
    ...
    self.x <- positions
    Population에서 Ds 구할 때는 self.get_distance로
    ...
    def __init__(self,
        ...
        self._label = label if label is not None else "".join([choice(ascii_letters) for i in range(10)])
        self.label_generation = label_generation
        # TODO: self.history 를 어떻게 두는 게 제일 좋을까? FA 알고리즘 전체의 관점이 아니라, agent 각자의 관점에서 보면 좋을 것 같은데
        self._fitness = None
    ...
    def evaluate(self):
        if self._fitness is None:
            self._fitness = self._x.get_potential_energy()
    ...
    def get_distance(agent_A, agent_B) -> float:
        ...
        '''
        return np.linalg.norm(np.sum(agent_A.desc, axis=0) - np.sum(agent_B.desc, axis=0))
...
class ASEAgent_internal_localopt(ASEAgent):
    ...
    TODO - 여기 GNN node_features_inv 넣어주어야 함 그게 없네
    '''
    def __init__(self,
                 save_local_opt = True,
                 save_local_opt_directory_name = "local_opts",
...
class FA:
    def __init__(self,
                 ...
                 gamma:float = 0.01,  # absorption coefficient
...
< Firefly algorithms parameters >
    ...
    * verbosity = {self.verbosity} # 0(error) < 1(warning) < 2(info) < 3(debug) \n\n
    ...
    def evaluate(self):
        ...
        if gbest_renewed:
            this_best_agent = self.current_gen.best_agent
 ...
 # ({self.current_gen.best_agent.label})\n\n")
    ...
    def evolve(self, frozen_index = None) -> bool:
        ...
        # Is -= Is.min()  # Use relative value instead of absolute value
        # Is /= Is.max()
        # Ds = self.current_gen.Ds
        # _bi, _bj = np.unravel_index(indices=np.argmax(Ds), shape=Ds.shape)
        # r_real = np.linalg.norm(self.current_gen[_bj] - self.current_gen[_bi])
        # gamma_eff = self.gamma * r_real ** 2
        # As = Is * np.exp(-gamma_eff * (Ds / Ds.max()) ** 2)
        #
        # idx_Is = np.array(list(range(self.population_size)), dtype=int)
        # idx_Js = np.argmax(As, axis=1)  # maximize the attract
        #
        # _dxes = []
        # debug_mes = ""
        # self.n_gen += 1
        # New_agents = []
        # all_total_dx, all_raw_dx, all_rand_dx = [], [], []
        # for i, j in zip(idx_Is, idx_Js):
        #     if i in frozen_index:
        #         agent_i = self.current_gen[i]
        #         agent_i.label_generation = f"Gen_{self.n_gen}"
        #         debug_mes += f"\n {agent_i.label} doesn't move. [freeze]"
        #         total_dx = dx = rand_dx = np.zeros(self.current_gen[i].x.shape)
        #     else:
        #         agent_i = self.current_gen[i]
        ...
        #         else:
        #             dx = (agent_j - agent_i)
        #             dx *= self.beta_0 * np.exp(-self.gamma * np.linalg.norm(dx) ** 2)
        #         _dxes.append(dx)
        ...
        #                                          )
        #         debug_mes += f"\n Gen_{self.n_gen}: Agent_{agent_i._label} moves towards Agent_{agent_j._label}, "\
        #                      f"dx: {total_dx} (beta*(xj-xi): {dx} + random_walk: {rand_dx})"
        ...
        Ds = self.current_gen.Ds
        _bi, _bj = np.unravel_index(indices=np.argmax(Ds), shape=Ds.shape)
        As = Is*np.exp(-self.gamma*(10*Ds/Ds.max())**2) # set r_max as 10

        idx_Is = np.array(list(range(self.population_size)), dtype=int)

        idx_Js = []
        for i, lis in enumerate(As):
            j_cands = np.argsort(lis)[::-1]
            for _ in j_cands:
                if _ not in frozen_index:
                    j = _
                    break
            idx_Js.append(j)
        idx_Js = np.array(idx_Js, dtype=int)
        # idx_Js = np.argmax(As, axis=1) # maximize the attraction

        self.add_log("\n\n>>> (+) Distance matrix", level=3)
        self.add_log(Ds, level=3)

        self.add_log("\n\n>>> (+) Attraction matrix by Intensity & distance", level=3)
        self.add_log(As, level=3)

        self.add_log(f"\n\n< Agent moving trajectories >", level=3)
        _dxes = []
        # move_towards = []
        debug_mes = ""
        self.n_gen += 1
        ...
        for i, j in zip(idx_Is, idx_Js):
            if i in frozen_index:
                ...
                agent_i = self.current_gen[i]
                agent_j = self.current_gen[j]
                # move_towards.append(agent_j.label)
                if i==j:
                    dx = np.zeros(self.current_gen[i].x.shape)
                else:
                    # dx = (agent_j - agent_i) * _beta
                    dx = (agent_j - agent_i)
                    # dx *= self.beta_0*np.exp(-self.gamma * np.max(np.linalg.norm(dx, axis=1))**2)
                    # dx *= self.beta_0 * np.exp(-self.gamma * np.linalg.norm(dx)**2)
                    dx *= self.beta_0 * np.exp(-self.gamma * (dx**2).sum()) # same but more efficient
                _dxes.append(dx)
                agent_i.label_generation = f"Gen_{self.n_gen}" # For ASEAgent, evaluation is performed during the 'move'
                total_dx, rand_dx = agent_i.move(dx,
                                                 alpha = self.alpha,
                ...
                self.add_log(f"\n Gen_{self.n_gen}: Agent_{agent_i._label} moves towards Agent_{agent_j._label}", level=2)
            ...
            all_raw_dx.append(dx)  # related to beta_0
    ...
    def autorun_gbest_only(self,
        ...
        while True:
            ...
            self.save_population("ongoing.pickle")
            if self.n_gen != max_n_gen:
    ...
    def autorun_locals(self,
        ...
        while True:
            ...
            if cluster_counters and min(cluster_counters.values()) > patience_for_locmins:
                self.add_log(f"Local convergence achieved after generation {self.n_gen}.", level=2)
                return True
    ...
    def save_population(self, restart_filename=None):
        agent_states = []
    ...
    def load_population(self, restart_filename=None):
...
# TODO: fit_lr_valid -> 이거 나중에 gnn_LL 로 옮기자
@torch.no_grad
...
class FA_AL(FA):
    def __init__(self,
                 ...
                 restart: Optional[str] = "ongoing.pickle",  # filename of pickle file  # TODO - DFT data:xyz 파일 읽는 것도 넣어두자
        ...
        # TODO: 이전 evolve에 있어서 대충 써뒀는데 놓친 거 있을까?
        self.use_levy = False
        self.levy_scale = 0.0
        ...
        * verbosity = {self.verbosity} # 0(error) < 1(warning) < 2(info) < 3(debug) \n\n
        ...
        if (not _xyz_loaded) or (not os.path.exists(self.gnn_filename)):
            ...
            for _ in range(self.n_init):
                ...
                self.add_log(f"Entering the SCF calc of {new_agent.label}", level=0) # TODO: level=3으로 바꿀거
                if os.path.exists("scf.traj"):
                    ...
                    # TODO: VASP calculator -> SinglePoint Calculator로 바꾸기. 이거 더 좋은 방법 있을텐데? calculator만 바꾸는 걸로
                    _ = Trajectory("scf.traj", 'r')
                    model = list(_)[-1]  # model is changed to already-calculated structure
                    E = model.get_potential_energy()
                    _.close()
        ...
        self.add_log(f"Generating Dataset", level=0)  # TODO: level=3으로 바꿀 것
        ...
        self.add_log(f"Check restart file", level=0)
        if restart is not None:
            if isinstance(restart, str):  # filename
                if os.path.exists(restart):
                    self.add_log(f">>> Read initial generation & lowest minima from {restart} files!\n\n", level=2)
        ...
        if self.current_gen is None:
            self.add_log(f"Generation not loaded, random agents generated", level=0)
            self.n_gen = 1
            self.gbest_history = []
            ...
            # GNN-load 부터 시작이 아닌 이상, force training을 한 번 해줍니다. 근데 전부 다 돌리기는 좀 그러니까, clustering 후에 각 cluster 별 최저값으로 진행
            if not load_gnn:
                # naive geo-opt, since GNN0 is not robust enough
                self.add_log("Enter generating force_init", level=0) # TODO: 이후 level=3으로 바꾸기
                ...
                self.add_log(f"Retrain requested", level=0)
            ...
            self.add_log("Initiating Population", level=0)
                ...
                if self.lcb_tracker.check(agent.atoms):
                    ...
                    for agent in self.waiting_for_target_calc:
                        ...
                        if not os.path.exists(os.path.join(fn, fnn)):
                            ...
                            # model.info = {}
                            # model.arrays
                            write_vasp(fnn, agent.atoms, direct=True, sort=False)
                    self.frozen_agent_index.append(i) 
