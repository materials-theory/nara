...
class VAE_2L_ATTN(nn.Module):
    ...
    def train_epoch(self, dataloader, optimizer, device, recon_only=False):
        ...
        for data, in dataloader:
            ...
            if recon_only:
                ...
                loss = loss_function_ELBO(x_recon, self.attn_pool(data), mu, logvar)
            loss.backward()
            optimizer.step()
    ...
    @torch.no_grad()
    def validate_epoch(self, dataloader, device, recon_only=False):
...
def main(device="cpu", dtype="float64"):
    ...
    for i in range(34):
        ...
        for _x in _x_ab:
            X_locals.append(_x) # (N_atoms, input_dim)
    ...
    for epoch in range(max_epoch):
        st = time.time()
        avg_train_loss = model.train_epoch(train_loader, optimizer, device, recon_only = recon_only)
        avg_val_loss = model.validate_epoch(val_loader, device, recon_only = recon_only)
        if patience is None:
