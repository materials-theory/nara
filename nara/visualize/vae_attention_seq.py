...
class VAE_2L_ATTN(nn.Module):
    ...
    def train_epoch(self, dataloader, optimizer, device, recon_only=False, beta=1.0):
        self.train()
        total_loss = 0
        for data, in dataloader:
            ...
            if recon_only:
                ...
                x_recon = self.encoder(_ref_recon)
                x_recon = self.mean_layer(x_recon)
                x_recon = self.decoder(x_recon)
                loss = loss_function_recon(x_recon, _ref_recon)
            else:
                x_recon, mu, logvar = self.forward(data)
                loss = loss_function_ELBO(x_recon, self.attn_pool(data), mu, logvar, beta=beta)
            loss.backward()
            optimizer.step()
    ...
    @torch.no_grad()
    def validate_epoch(self, dataloader, device, recon_only=False, beta=1.0):
        self.eval()
        total_loss = 0
        for data, in dataloader:
            data = data.to(device)
...
def loss_function_ELBO(x_recon, x, mu, logvar, beta = 1.0):
    recon_loss = F.mse_loss(x_recon, x, reduction='sum')
    KL_divergence = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    loss = recon_loss + beta * KL_divergence  # Total loss = reconstruction loss + KL divergence
    return loss
...
def main(device="cpu", dtype="float64"):
    ...
    for i in range(34):
        ...
        for _x in _x_ab:
            X_locals.append(_x) # (N_atoms, input_dim) 텐서들을 list로 처리
    ...
    for epoch in range(max_epoch):
        st = time.time()
        avg_train_loss = model.train_epoch(train_loader, optimizer, device, recon_only = recon_only, beta=2.0)
        avg_val_loss = model.validate_epoch(val_loader, device, recon_only = recon_only, beta=2.0)
        if patience is None:
